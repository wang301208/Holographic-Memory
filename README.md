...
全息记忆存储（Holographic Memory）— 基于 FFT 的高容错知识表示与检索引擎

## 概述

本项目实现了一套完整的全息记忆存储系统，基于卡尔·普里布拉姆的全息脑模型和丹尼斯·加博尔的全息术理论，将知识通过傅里叶变换映射到频域，使得每个存储片段都包含整体信息的缩影，从而实现极强的容错性和联想检索能力。
...
|------|-------------------|----------|
| 容错性 | 低（丢失即丢失） | 高（冗余编码 + 校验片段 + RS纠删） |
| 联想能力 | 依赖相似度计算 | 天然支持全息联想 |
| 存储效率 | 1x | 0.3x（压缩比更高） |
| 50%损毁恢复 | 不可能 | 可恢复 |
| 纠删编码 | 无 | GF(2^8) Reed-Solomon |
| 磁盘索引 | 单层 | 分层 L0/L1/L2 |
